...
class ThreeWavesUp:
    ...
    def update_bar(self, bar):
        ...
        if segment_end and len(self.bars) >= 3:
            ...
            if len(self.segments) == 3 and list(self.segment_trends) == ['up', 'down', 'up']:
                if self.segments[0][2] >= 6 and self.segments[1][2] >= 3 and self.segments[2][2] >= 6:
                    if self.segments[0][0]>1.1*abs(self.segments[1][0]) and self.segments[2][0]>1.1*abs(self.segments[1][0]):
                        self.segments.clear()
