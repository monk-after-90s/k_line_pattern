'''
调用方法如下：
pattern_analyzer = XiangTiZhengLi(data，interval)
...
class XiangTiZhengLi:
    ...
    def analyse_pattern(self):
        ...
        if min(abs(slope_high), abs(slope_low)) / max(abs(slope_high), abs(slope_low)) >= 0.75 \
                and slope_low * slope_high >= 0:
